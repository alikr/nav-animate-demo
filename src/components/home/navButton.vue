		...
		<g :transform="translate" class="out">
			...
			<g class="outline">
				...
				<g mask="url(#maskLine)" class="maskLine">
					<template v-for='(item, index) in outLines' >
						<path fill='#32A32A' stroke="#44DB39" :d="item"></path>
					</template>
				</g>
	...
	export default {
		data:function(){
			return {
				...
				dots:[],//里圈短线坐标
				dotsRotate:[{rotate:0,step:this.getRandomStep(),_rotate:'rotate(0)'}],//里圈短线旋转
				outDots:[],//外圈短线坐标
				outLines:[],//外圈长线坐标
				outLines2:[],//外圈长线坐标
				arcs1:[],//里圈小弧线
				arcs2:[],//里圈大弧线
				arcs3:{d:'',rotate:0,step:this.getRandomStep()},//外圈弧线1
				arcs4:{d:'',rotate:0,step:this.getRandomStep()},//外圈弧线2
				arcs5:{d:'',rotate:0,step:this.getRandomStep()},//外圈弧线3
				arcs3_line:'',//外圈弧线1的线
				arcs4_line:'',//外圈弧线2的线
				arcs5_line:'',//外圈弧线3的线
				translate:'',//平移到中心
				outArc:'',//外层宽弧线
				triangle1:{d:'',transform:'',x:0,y:0},
				triangle2:{d:'',transform:'',x:0,y:0},
				navs:[],
				areaPath:''
			}
		},
	    created() {
			...
			for(var i = 0;i < dots; i++){
				...
				_this.outLines.push('M'+x1+','+y1+'L'+x2+','+y2+'L'+x4+','+y4+'L'+x3+','+y3+'Z');
			};
			...
			for(var i = 0;i < arcs1; i++){
				...
				_this.arcs1.push({d:d,rotate:0,step:step,_rotate:'rotate(0)'});
			}
			...
			for(var i = 0;i < arcs2; i++){
				...
				var d = arc.innerRadius(minr + 20)
				    ...
				    .endAngle(start + angle)();
				var step = _this.getRandomStep();
				_this.arcs2.push({d:d,rotate:0,step:step,_rotate:'rotate(0)'});
			}
		...
		methods: {
			...
			getRriangle(r,angle){
				...
				return {d:path,transform:'translate(0,0)',x:0,y:0};
			},
			navCircle(index){
				var arcs2 = 3;
				var minr = 30;
				...
				return {d:nav_arc+nav_circle,translate:'translate('+cx+','+cy+')'};
			},
			setNav(){
				for(var i=1;i<4;i++){
			...
			setGauge(){
		        ...
		        for (var i = 0; i <= total; i++) {
		            data.push({ x: i, y: (40+Math.random()*maxR) });
		        }
	            ...
	            var line = d3.radialArea()
				    ...
				    .outerRadius(function(d){return radius(d.y+maxR)})
				    .angle(function(d){return angle(d.x)})
				    .curve(d3.curveBasisClosed);
				var _preAreaPath = _.extend(this.areaPath);
			...
			updateOutLines2(flag){
				...
				for(var i = 0;i < dots; i++){
					...
					var item = {x1,y1,x2,y2,dash:dashLen+','+dashGap};
